...
{
    /**
     * @Route("/new_global_message", name="new_global_message")
    ...
    /**
     * @Route("/show_global_messages/{access}", name="show_global_message")
     * @param int $access
     * @return JsonResponse
     */
    public function showGlobalMessage(int $access):  JsonResponse
    {
        $product = $this->getDoctrine()
            ->getRepository(GlobalMessage::class)
            ->findBy(
                ['destinator_id' => $access]
            );
        $data=[];
        ...
        return new  JsonResponse(['data' => $data, 'message'=>'Messages for the employee with access '.$access,'success'=> true]);
    }
}
